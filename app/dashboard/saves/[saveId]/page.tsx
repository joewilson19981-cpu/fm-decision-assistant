    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <h1 className="text-2xl font-bold text-gray-900">{save.name}</h1>
      ...
      {latest ? (
        <div className="bg-white rounded-xl border border-gray-200 p-5 mb-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-semibold text-gray-800">Current Snapshot</h2>
            <Link
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div>
              <p className="text-xs text-gray-500 mb-1">In-game date</p>
              <p className="font-semibold text-gray-900">{formatDate(latest.inGameDate) ?? '—'}</p>
            </div>
            <div>
              <p className="text-xs text-gray-500 mb-1">Season</p>
              <p className="font-semibold text-gray-900">{latest.season.seasonLabel}</p>
            </div>
            <div>
              <p className="text-xs text-gray-500 mb-1">Transfer window</p>
              <p className="font-semibold text-gray-900">{latest.transferWindowStatus ?? '—'}</p>
            </div>
            <div>
              <p className="text-xs text-gray-500 mb-1">Calendar phase</p>
              <p className="font-semibold text-gray-900">{latest.calendarPhase ?? '—'}</p>
            </div>
          </div>

          {ts && (
            <>
              <div className="border-t border-gray-100 mt-4 pt-4 grid grid-cols-3 sm:grid-cols-6 gap-3 text-center">
                {[
                  ...
                  <div key={stat.label} className="bg-gray-50 rounded-lg p-2">
                    ...
                    <p className="font-bold text-gray-900 text-sm">{String(stat.value)}</p>
                  </div>
                ))}
              </div>
            </>
        ...
        <div className="bg-gray-50 border border-dashed border-gray-300 rounded-xl p-6 mb-6 text-center">
          <p className="text-sm text-gray-500">No checkpoints yet. Create a season and add a checkpoint to see your save snapshot here.</p>
        </div>
      )}
      ...
      {save.philosophy && (
        <div className="bg-white rounded-xl border border-gray-200 p-5 mb-6">
          <h2 className="font-semibold text-gray-800 mb-3">Save Philosophy</h2>
          <div className="grid grid-cols-2 gap-3 text-sm">
            {(save.philosophy as any).preferredFormation && (
              <div>
                <span className="text-gray-500">Formation: </span>
                <span className="text-gray-900">{(save.philosophy as any).preferredFormation}</span>
              </div>
            )}
            {(save.philosophy as any).recruitmentStyle && (
              <div>
                <span className="text-gray-500">Recruitment: </span>
                <span className="text-gray-900">{(save.philosophy as any).recruitmentStyle.replace(/_/g, ' ')}</span>
              </div>
            )}
            {(save.philosophy as any).longTermGoal && (
              <div className="col-span-2">
                <span className="text-gray-500">Goal: </span>
                <span className="text-gray-900">{(save.philosophy as any).longTermGoal}</span>
              </div>
            )}
          </div>
        </div>
      ...
      <div>
        ...
        {save.seasons.length === 0 ? (
          <div className="bg-white rounded-xl border border-gray-200 p-10 text-center">
            ...
            <h3 className="text-lg font-semibold text-gray-800 mb-2">No seasons yet</h3>
            <p className="text-sm text-gray-500 mb-4">Add your first season to start tracking your progress.</p>
            <Link
              href={`/dashboard/saves/${save.id}/seasons/new`}
              className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-blue-700"
            >
              ...
              return (
                <Link key={season.id} href={`/dashboard/saves/${save.id}/seasons/${season.id}`}>
                  <div className="bg-white rounded-xl border border-gray-200 p-5 hover:border-blue-300 hover:shadow-sm transition-all cursor-pointer">
                    ...
                    {sts && (
                      <div className="flex gap-4 mt-3 pt-3 border-t border-gray-100 text-sm text-gray-600">
      ...
      {save.notes && (
        <div className="bg-white rounded-xl border border-gray-200 p-5 mt-6">
          ...
          <p className="text-sm text-gray-600 whitespace-pre-wrap">{save.notes}</p>
        </div>
      )}
    </div>
  )
