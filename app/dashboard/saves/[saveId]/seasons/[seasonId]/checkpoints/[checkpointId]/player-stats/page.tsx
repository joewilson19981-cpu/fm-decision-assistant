  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-gray-500 text-sm mt-1">Enter stats for each player. Leave blank anything you don't have.</p>
      </div>
      ...
      <form onSubmit={handleSubmit}>
          ...
          {players.map((player, i) => (
            <div key={i} className="bg-white rounded-xl border border-gray-200 p-4">
              <div className="flex items-center justify-between mb-3">
                <span className="text-xs font-semibold text-gray-500 uppercase">Player {i + 1}</span>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-1">Contract expiry</label>
                  <input type="date" value={player.contractExpiry} onChange={e => updatePlayer(i, 'contractExpiry', e.target.value)}
                    className="w-full border border-gray-300 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-1">Morale</label>
        ...
        <button type="button" onClick={addPlayer} className="text-sm text-blue-600 hover:underline mb-6">+ Add player</button>
