    ...
    <div className="max-w-2xl">
      <div className="mb-6">
        <div className="flex items-center gap-2 text-sm text-zinc-500 mb-1">
          ...
          <span>/</span>
          <Link href={`/dashboard/saves/${saveId}/seasons/${seasonId}`} className="hover:text-zinc-300">Season</Link>
          <span>/</span>
          <span>New Checkpoint</span>
        </div>
        <h1 className="text-2xl font-bold text-white">New Checkpoint</h1>
        <p className="text-zinc-500 text-sm mt-1">Capture a snapshot of where your save is right now.</p>
      </div>

      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="rounded-xl card-panel border border-white/[0.06] p-5 space-y-4">
          <h2 className="font-semibold text-white">Checkpoint Details</h2>

          <div>
            <label className="block text-sm font-medium text-zinc-300 mb-1">Type <span className="text-red-500">*</span></label>
            <select
          ...
          <div>
            <label className="block text-sm font-medium text-zinc-300 mb-1">Notes <span className="text-zinc-600 font-normal">(optional)</span></label>
            <textarea
              value={form.notes}
