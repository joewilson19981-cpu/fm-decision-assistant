  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-gray-500 text-sm mt-1">Things to do across your saves</p>
      </div>

      {items.length === 0 ? (
        <div className="bg-white rounded-xl border border-gray-200 p-10 text-center">
          <div className="text-4xl mb-3">📝</div>
          <h2 className="text-lg font-semibold text-gray-800 mb-2">No action items yet</h2>
            ...
            groupItems.length === 0 ? null : (
              <div key={status}>
                ...
                <div className="space-y-2">
                  {groupItems.map(item => (
                    <div key={item.id} className="bg-white rounded-xl border border-gray-200 p-4 flex items-start justify-between">
                      <div>
                        <h3 className="font-medium text-gray-900">{item.title}</h3>
                        {item.description && <p className="text-sm text-gray-500 mt-0.5">{item.description}</p>}
                        <p className="text-xs text-gray-400 mt-1">{item.save.name}{item.season && ` · ${item.season.seasonLabel}`}</p>
                      </div>
